package com.example.persona.migration.model.oracle;

import jakarta.persistence.Column;
import jakarta.persistence.Entity;
import jakarta.persistence.Id;
import jakarta.persistence.IdClass;
import jakarta.persistence.Table;
import java.math.BigDecimal;
import java.time.LocalDateTime;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.NoArgsConstructor;

@Entity
@Table(name = "MTX_VIRTUAL_CARD")
@IdClass(MtxVirtualCardId.class)
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class MtxVirtualCard {

    @Id
    @Column(name = "MASTER_ACC_ID", length = 20, nullable = false)
    private String masterAccId;

    @Id
    @Column(name = "TRACKING_NUMBER", length = 20, nullable = false)
    private String trackingNumber;

    @Id
    @Column(name = "CARD_TYPE", length = 10, nullable = false)
    private String cardType;

    @Column(name = "CREATED_BY", length = 20)
    private String createdBy;

    @Column(name = "CREATED_ON")
    private LocalDateTime createdOn;

    @Column(name = "MODIFIED_BY", length = 20)
    private String modifiedBy;

    @Column(name = "MODIFIED_ON")
    private LocalDateTime modifiedOn;

    @Column(name = "CARD_STATUS", length = 2)
    private String cardStatus;

    @Column(name = "CARD_NUMBER", length = 4)
    private String cardNumber;

    @Column(name = "EMAIL", length = 50)
    private String email;

    @Column(name = "PHONE_NUMBER", length = 20)
    private String phoneNumber;

    @Column(name = "REASON_ID", length = 20)
    private String reasonId;

    @Column(name = "REMARKS", length = 200)
    private String remarks;

    @Column(name = "EXPIRY_DATE", length = 10)
    private String expiryDate;

    @Column(name = "SC_ENABLED", length = 1)
    private String scEnabled;

    @Column(name = "MAX_DLY_PUR_AMT", precision = 25, scale = 10)
    private BigDecimal maxDlyPurAmt;

    @Column(name = "DLY_PUR_LIM", precision = 25, scale = 10)
    private BigDecimal dlyPurLim;

    @Column(name = "ENTITY_CODE", length = 20)
    private String entityCode;

    @Column(name = "ACCOUNT_NO", length = 20)
    private String accountNo;
}
